...
> Je m'appelle, **Alain Guillon** alias **Zyrass**, je n'ai aucune affiliation avec la plateforme de Dyma.
> En revanche, j'ai une très grande admiration pour ce qui a été fait jusqu'à maintenant.
> Pour les remercier, je vous propose à vous chèr(e)s étudiant(e)s un exercice pour réviser les quatre premiers chapitres.

## Sommaire

> Certaines étapes risquent pour certains de poser quelques soucis. Pour combler tout ça,
...
- /!\ Git ne posera pas de soucis pour la continuité de l'exercice.
```
...
7. Ajout de quelques **scripts** dans ce fichier (`create`, `read`, `update`, `delete`)... _Du **C.R.U.D** avec **FS**_
8. Création de l'architecture (réfléchissez où idéalement on placerait ce genre de création)
...
> **languages** est le nom que j'ai donné au dossier principal. (Celui qui accueillera tous les sous-dossiers)

10. Définition du problème à résoudre

-   Pour vous aider à y voir plus clair, j'ai créé un fichier symaptique copier l'intégralité du code dans le fichier **app.js**
...
const probleme = `
  ...
  Je sais que tu en es capable, tu es inscrit sur DYMA donc tu gères!!
  Mais je me demande si vraiment tu aurais un peu de temps à me consacrer!

  En effet, le temps me manque... Bon allez, je m'enfou je t'informe tout de même du problème.
  
  J'ai un client qui m'harcèle pour que je lui crée un dossier qui contiendrait : 
  ...
  Si tu peux me faire ça tout en automatisé, ce serait top !!
  Comme ça si cet endouille supprime son dossier avec tout son contenu,
  ...
  ps: Je t'ajoute un objet qui t'aidera pour la réalisation du projet`;
...
```diff
- Il faut savoir qu'il vous faudra aller voir la documentation officielle de node
- Pour ma part j'ai utiliser du code qui n'a pas été présenté.
```

1. Par exemple avec la détection d'un dossier s'il existe déjà ou non
2. Un listing de tous les répertoires et donc contrôler l'existence de telle ou telle chose.

## Pour les plus téméraires

Une fois que j'aurais ajouté mon code

- Vous pouvez vous amuser à faire toutes les vérifications possibles pour éviter l'affichage de quelques erreurs
